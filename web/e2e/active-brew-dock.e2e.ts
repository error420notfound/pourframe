...
async function startMockBrew(page: import('@playwright/test').Page) {
  ...
  await page.getByRole('region', { name: 'Choose your brew' }).getByRole('button', { name: 'Prepare brew' }).click()
...
test('keeps active brew navigation and controls available outside focus view', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Prepare brew' })).toBeDisabled()
