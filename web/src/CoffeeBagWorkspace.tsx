...
export function CoffeeBagWorkspace({ bags, onSave, onDelete, onUse }: { bags: CoffeeBagRecord[]; onSave: (bag: CoffeeBagRecord) => Promise<void>; onDelete: (id: string) => Promise<void>; onUse: (id: string) => void }) {
  ...
  return <section className="library-workspace coffee-library-workspace">
    <PageHeader actions={<Button className="new-recipe" onClick={() => setEditor(createCoffeeBag())} variant="secondary"><Plus aria-hidden="true" />Add bag</Button>} className="library-workspace__header" description="Keep the coffees you have on hand ready for your next brew." eyebrow="Shared inventory" title="Beans" variant="library" />
