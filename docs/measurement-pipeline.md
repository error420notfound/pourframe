...
The Arduino loop only copies the latest immutable snapshot. LED rendering, JSON serialization, Wi-Fi maintenance, and WebSocket transmission never run in the sensor task. Production telemetry remains protocol v1, retains the existing fields, adds measurement diagnostics, and publishes the latest snapshot every 100 ms (10 Hz) without bucket averaging. Slow clients receive newer frames instead of a queued backlog.
